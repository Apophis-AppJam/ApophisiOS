...
//        appData?.chatIndex = index
    ...
    @objc func setHandBackground()
    {
        
        messageTextInputView.backgroundColor = .clear
    ...
    func loadDummyMessage(idx : Int,isMine : Bool)
    {
        ...
        else if idx == 1
        {
            if isMine == false
            {
                ...
                isMessageLoadList.append(contentsOf: [false, false, false, false])
            }
            else
            {
                newMessageList.append(contentsOf: [
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if newMessageList[indexPath.row].isMine == true // 나의 메세지인 경우
        {
            ...
            case .enterName:
                ...
                guard let enterNameCell = tableView.dequeueReusableCell(withIdentifier: "Day3EntetNameCell", for: indexPath)
                        as? Day3EntetNameCell
                else { return UITableViewCell() }
